...
if len(sys.argv)<=1:
    print("======== Usage =========")
    print("python BatchMergeTruthAndProcessed_peaks.py <config file> <truth root path> <processed root path> <output path> <(opt)top-to-total fraction in truth(default 0.68)> <(opt)relative path for submission> <(opt) if use public node (1) optional (2 for use kicp nodes)> <(opt)Submit ID>")
    exit()
...
EXE_Path = CurrentEXE.split("BatchMergeTruthAndProcessed_peaks.py")[0]
EXE1 = CurrentPath+"/"+EXE_Path+"TruthSorting_peaks.py"
...
FileList = glob.glob(ProcessedRootPath+"/*.root")
# trail to IDList
IDList = []
for filename in FileList:
    ID = filename.split("FakeWaveform_XENON1T_")[1].split("_pax_")[0]
    IDList.append(ID)
...
for j, ID_job in enumerate(IDList):
    ...
    while IfSubmitted==0:
        ...
        Status=subp.call("squeue --partition="+Partition+" --user=feigao | wc -l", shell=True)
        Output=int(output)
        #print(Status)
